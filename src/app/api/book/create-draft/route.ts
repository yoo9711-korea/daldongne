  ...
  try {
    ...
    const bookDirectionPrompt = [
      ...
      '위 제작 방향을 반드시 반영해서 원고를 작성하세요.',
        ...
        const essayWritingPrompt = [
      '감성 에세이 작성 원칙:',
      '- 화려한 수식어와 억지스러운 감동을 사용하지 마세요.',
      '- “엄청난”, “기적처럼”, “운명처럼”, “마침내 깨달았다”, “우주가 도왔다”, “세상이 멈춘 듯했다”, “눈물이 왈칵 쏟아졌다” 같은 과장되거나 익숙한 클리셰 표현을 사용하지 마세요.',
      '- 담담하고 고요한 한국어 산문으로 작성하세요.',
      '- 문장 끝은 “~했습니다”, “~했다”, “~였습니다” 등을 문맥에 맞게 자연스럽게 사용하세요.',
      '- 감정을 직접 선언하기보다 인물의 작은 행동, 표정, 말의 간격, 공간의 분위기, 오래된 물건과 일상의 장면으로 보여주세요.',
      '- 사용자가 적지 않은 감정, 관계, 사건, 대화, 장소, 계절, 날씨를 추측하거나 만들어 내지 마세요.',
      '- 자료가 부족한 부분은 의미를 부풀리지 말고 조용히 비워 두거나, 앞으로 더 기록할 수 있는 여지로 남기세요.',
      '- 같은 감정과 의미를 표현만 바꾸어 반복하지 마세요.',
      '- 독자를 울리려는 문장보다 한 사람의 시간을 정확하고 따뜻하게 바라보는 문장을 작성하세요.',
      '- 제목, 부제, 요약, 표지문구도 과장 없이 실제 자료의 핵심을 담아 작성하세요.',
      '',
      '고전 문구 인용 원칙:',
      '- 인용은 본문 전체에서 최대 한 번만 사용하세요.',
      '- 제목, 부제, 요약, 표지문구에는 인용문을 사용하지 마세요.',
      '- 이야기의 상황과 자연스럽게 맞는 경우에만 아래 허용 목록에서 한 문구를 원문 그대로 사용하세요.',
      '- 문맥에 맞는 문구가 없으면 억지로 인용하지 마세요.',
      '- 허용 목록 밖의 문구, 한국어 번역문, 출처가 불확실한 문구를 새로 만들거나 인용하지 마세요.',
      '- 문구의 단어와 출처를 임의로 바꾸지 마세요.',
      '',
      '허용 인용 목록:',
      '1. “The sun is but a morning star.” — Henry David Thoreau, Walden',
      '2. “Love looks not with the eyes, but with the mind.” — William Shakespeare, A Midsummer Night’s Dream',
      '3. “All the world’s a stage.” — William Shakespeare, As You Like It',
    ].join('\n');
      ...
      input: [
        {
          role: 'system',
          content:
            '당신은 달동네 출판사의 인생책 편집장이자 담담한 감성 에세이 작가입니다. 사용자가 남긴 사진과 이야기에서 확인되는 사실만 사용하여 시간의 흐름을 따라 읽히는 책 원고를 작성합니다. 화려한 수식어로 감정을 과장하지 않으며, 자료에 없는 사실이나 감정을 지어내지 않습니다.',
        },
        {
          role: 'user',
          content: [
